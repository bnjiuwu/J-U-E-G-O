...
@export var next_level_path: String = "res://Assets/Scenes/Menu/menu.tscn" # Ajusta si tienes un nivel 4
...
func _ready():
	# --- DEBUG + FORZAR SIN LOOP (Igual que Nivel 1) ---
	...
	# Inicializar paredes del boss ocultas (Patrón Nivel 1)
	if boss_walls:
		boss_walls.visible = false
		boss_walls.collision_enabled = false

	# ================================
	#  MENÚS: Delegar a LevelManager
	# ================================
	...
	# Fallback por grupo si no hay manager o no se asignaron
	if pause_menu == null:
	...
	if player and death_menu:
		...
		print("❌ Error: No se encontró el player o el death menu en Level 3")
...
func _handle_level_completion() -> void:
	...
	# ✅ SOPORTE MULTIJUGADOR (Igual que Nivel 1)
...
func _on_player_died() -> void:
	# ✅ Si es multi, deja que LevelManager maneje las vidas/derrota
...
func _on_pause_button_pressed():
	print("🟢 Señal recibida en level_3 → abrir menú")
...
func _on_animation_finished(anim_name: StringName) -> void:
	if anim_name == &"fade":
		...
		if $player/Camera2D/Label2:
			$player/Camera2D/Label2.visible = false
